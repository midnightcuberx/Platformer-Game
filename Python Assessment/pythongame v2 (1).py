...
class BasicView(arcade.View):
    ...
    def on_show(self):
        ...
        # Sets background colour of the instruction panel
...
class InstructionView(BasicView):
    ...
    def on_mouse_press(self, _x, _y, _button, _modifiers):
        """This starts the game if the user pressed the button"""
...
class LevelPassed(BasicView):
    ...
    def __init__(self, game_view, time:float):
    ...
    def on_draw(self):
        arcade.draw_text(f"Congrats for passing level {self.game_view.level-1}!", self.window.width / 2, self.window.height / 2,
                         arcade.color.WHITE, font_size=50, anchor_x="center")
        arcade.draw_text(f"You passed the level with a score of {self.game_view.player_score} and a time of {self.time}", self.window.width / 2, self.window.height / 2-75,
                         arcade.color.WHITE, font_size=20, anchor_x="center")
...
class Entity(arcade.Sprite):
    ...
    def __init__(self,folder_name, file_name, scale : int, no_of_frames: int, idle_img, walking_img ,jumping_img = None, falling_img = None, climbing_img = None):
...
class Character(Entity):
    ...
    def __init__(self):
        ...
        # and takes on all the variables of it
        super().__init__("Ninja/png", "", CHARACTER_SIZE, 10,"Idle__000", "Run__00","Jump__001", "Jump__009", "Climb_00")
...
class Game(arcade.View):
    ...
    def setup(self):
        ...
        #enemies_layer=self.tile_map._process_object_layer(self.tile_map.object_lists[ENEMY_LAYER])
    ...
    def on_update(self, delta_time):
        ...
        if arcade.check_for_collision_with_list(
            ...
            self.character.center_x = self.character.center_x
            self.character.center_y = self.character.center_y

            window = self.window
            current_total_time = self.total_time
            # Calls level passed view which carries onto next level passing the game view as self
            pass_view = LevelPassed(self, current_total_time)
