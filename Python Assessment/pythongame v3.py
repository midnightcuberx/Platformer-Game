...
class Character(Entity):
    ...
    def __init__(self,moving_platforms):
        ...
        self.weapon_sprite = arcade.Sprite()
        ...
        for i in range(10):
            texture = load_textures(f"{MAIN_FILE_PATH}/Assets/Ninja/png/Attack__00{i}.png")
            self.attacking_textures.append(texture)
        
        
        # Keeps track of how many lives the character
        #self.lives = 3
    ...
    def check_for_attack_collsion(self, enemy_sprites: arcade.Sprite):
        hit_enemies = arcade.check_for_collision(self.weapon_sprite, enemy_sprites) # See if the weapon hits any enemies
        if hit_enemies:
            enemy_sprites.health -= random.randint(10,20)
    ...
    def update_animation(self, delta_time: float = 1 / 60):
        ...
        if self.is_attacking:
            self.texture_count()
            self.texture = self.attacking_textures[self.current_texture][self.character_direction]
            self.hit_box = self.attacking_textures[0][self.character_direction].hit_box_points
            #self.weapon_sprite.texture = self.weapon_textures[self.current_texture][self.character_direction]
            #self.weapon_sprite.hit_box = self.weapon_sprite.texture.hit_box_points
            #if self.character_direction == RIGHT_FACING:
                #self.weapon_sprite.left = self.right
            #else:
                #self.weapon_sprite.right = self.left
            
            #self.weapon_sprite.center_y = self.center_y
            self.current_texture += 1
            ...
            if self.change_y > 0 and not self.on_ladder and not self.on_moving_platform(self.moving_platforms):
                if not self.is_throwing:
                    ...
                    self.throwing_animation(self.jump_throw_textures, upper_limit=4)
                return
                ...
                if not self.is_throwing:
                    ...
                    self.throwing_animation(self.jump_throw_textures, 5)
                return
...
class SpeedPowerup(AnimatedObject):
    ...
    def __init__(self):
        ...
        # The hearts class becomes an animated object
        # and takes on all its variables
        super().__init__(f"{MAIN_FILE_PATH}Assets/Collectables/speed", HEART_HEAL_SIZE, 12,POWERUP_RATE)
...
class Game(arcade.View):
    ...
    def on_key_press(self, key, modifiers):
        ...
        elif key == arcade.key.SPACE and self.num_of_speed_boosts > 0 and self.speed_boost_equipped is False:
    ...
    def on_update(self, delta_time):
        ...
        for powerup in self.map.get_sprite_list(POWERUP_LAYER):
            ...
            if arcade.check_for_collision_with_list(self.character,self.map[POWERUP_LAYER]):
        ...
        # if enemies died
        for enemy in self.map.get_sprite_list(ENEMY_LAYER):
            self.character.check_for_attack_collsion(enemy)
            if enemy.health <= 0:
