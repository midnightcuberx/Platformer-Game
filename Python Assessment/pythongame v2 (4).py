    ...
    def update_animation(self, delta_time: float = 1 / 60, knife_throwing:bool = False):
        ...
        if self.change_y > 0 and not self.on_ladder:
            if not self.is_throwing:
                ...
                if self.current_texture > 4:
                    self.current_texture = 0
                self.texture = self.jump_throw_textures[self.current_texture][self.character_direction]
            return
        ...
        if self.change_x == 0:
            if not self.is_throwing:
                ...
                if self.current_texture > 9:
                    self.current_texture = 0
                self.texture = self.throwing_textures[self.current_texture][self.character_direction]
            return
...
class EnemyHealth(arcade.Sprite):
    ...
    def draw_health(self):
        ...
        arcade.draw_rectangle_filled(self.enemy.center_x, self.enemy.top + 10, ENEMY_HEALTH_WIDTH, 4 ,color= arcade.color.RED)
        health_width = ENEMY_HEALTH_WIDTH * self.enemy.health / MAX_HEALTH
        arcade.draw_rectangle_filled(self.enemy.center_x - 0.5 * ENEMY_HEALTH_WIDTH + 0.5 * health_width, self.enemy.top + 10, health_width, 4 ,color= arcade.color.GREEN)

class AnimatedObject(arcade.Sprite):
    """A class that animates objects such as coins and hearts/potions"""
    def __init__(self, file_path:str, scaling:int, num_of_frames:int):
...
class HealthPotion(AnimatedObject):
    ...
    def __init__(self):
        ...
        super().__init__(f"{MAIN_FILE_PATH}Assets/Collectables/heart", HEART_HEAL_SIZE, 6)
    ...
    """def __init__(self):
        ...
        file_path = f"{MAIN_FILE_PATH}Assets/Collectables/heart"
        ...
        self.texture = self.hearts_list[0]

    def update_animation(self, delta_time: float = 1 / 60):
        
        self.current_texture += 1
        self.texture = self.hearts_list[self.current_texture]"""
        
class KnifeIcon(AnimatedObject):
...
class Game(arcade.View):
    ...
    def setup(self):
        ...
        for checkpoint in self.map[CHECKPOINT_LAYER]:
            self.possible_checkpoints.append(checkpoint)
        #self.passed_checkpoints.append[self.last_checkpoint]
        self.checkpoint_index = 0
    ...
    """def on_mouse_motion(self, x, y, dx, dy):
        ...
        self.angle = math.atan((y-self.cam.centre_y - 325)/(x-self.cam.centre_x - 500))

        if x < self.character.center_x:
            self.character.character_direction = LEFT_FACING
    ...
    def on_update(self, delta_time):
        ...
        #checkpoint_list = arcade.check_for_collision_with_list(self.character, self.map[CHECKPOINT_LAYER])
        #for checkpoint in checkpoint_list:
        for checkpoint in self.map[CHECKPOINT_LAYER]:
            #if arcade.check_for_collision(self.character, checkpoint) and checkpoint not in self.last_checkpoint:
            #use checkpoint num to make sure that the checkpoint hasnot been passed (it has to be bigger than the checkpoint num)
            # also maybe use the fact that only if its nto ain a checkpoint list then it will become the last checkpoint
            if arcade.check_for_collision(self.character, checkpoint) and checkpoint not in self.passed_checkpoints:
                ...
                #self.last_checkpoint = checkpoint
                #self.checkpoint_index = self.checkpoint_dict[checkpoint]
                #if checkpoint not in self.passed_checkpoints:
                    #self.passed_checkpoints.append(checkpoint)
                """checkpoint_index = self.possible_checkpoints.index(checkpoint)
        ...
        if arcade.check_for_collision_with_lists(
            ...
            if self.hearts.lives > 0:
                ...
                if self.hearts.lives == 0:
                    ...
                    checkpoint = self.checkpoint_dict[last_checkpoint]
                    if last_checkpoint != checkpoint:
                        for key, value in self.checkpoint_dict.items():
                            if self.checkpoint_dict[key] == self.checkpoint_index:
                                last_checkpoint = key
                                break"""
                    last_checkpoint = self.last_checkpoint

                    """if self.possible_checkpoints.index(last_checkpoint) < self.checkpoint_index:

                        last_checkpoint = self.possible_checkpoints[self.checkpoint_index]"""
        ...
        if self.can_throw:
            ...
            if self.character.is_throwing:
                ...
                knife = KnivesAttack(img_src, KNIFE_SIZE, hit_box_algorithm= "Detailed")
