...
class BasicView(arcade.View):
    ...
    def on_show(self):
        ...
        # This resets the viewpoint back to the start of the level 
        # (the window will be viewed from the starting position 
        # of the game)
...
class InstructionView(BasicView):
    ...
    def on_draw(self):
        ...
        arcade.draw_text("Instructions Screen", self.window.width / 2,
            self.window.height / 2,arcade.color.WHITE, font_size=50,
            anchor_x="center")
        arcade.draw_text("Click to advance", self.window.width / 2,
            self.window.height / 2-75, arcade.color.WHITE, font_size=20,
            anchor_x="center")
    ...
    def on_mouse_press(self, _x, _y, _button, _modifiers):
        ...
        main_game = Game(STARTING_LEVEL,False)
        main_game.setup()
        self.window.show_view(main_game)
...
class LevelPassed(PauseView):
    ...
    def on_draw(self):
        ...
        arcade.draw_text(
            f"Congrats for passing level {self.game_view.level-1}!",
...
class GameOver(PauseView):
    ...
    def __init__(self, game_view, score:int, time:float):
        ...
        # Creates the colour which will be our 
        # background colour when the game is over
        fill_colour = arcade.color.RED_DEVIL
...
class Weapon(arcade.Sprite):
    ...
    def __init__(self,character):
        ...
        for i in range(10):
            texture = load_textures(MAIN_FILE_PATH +
                f"/Assets/Ninja/png/Attacking/Attack__00{i}.png")
    ...
    def check_for_attack_collision(self, enemy_sprites: arcade.Sprite):
        ...
        if hit_enemies and self.character.is_attacking:
            ...
            #Replace with weapon damage constant
    ...
    def update_animation(self, delta_time: float = 1 / 60):
        ...
        if self.character.is_attacking:
            ...
            # This changes the center_x of the weapon based on 
...
class Character(arcade.Sprite):
    ...
    def __init__(self,moving_platforms, character_type = NINJA_GIRL):
        ...
        for folder in folder_names:
            ...
            flying_textures = []
            for i in range(10):
                texture = load_textures(f"{file_path}Glide_00{i}.png")
                flying_textures.append(texture)
            ...
            self.flying_textures.append(flying_textures)
    ...
    def throwing_animation(self,texture_list,
        lower_limit:int = 0, upper_limit:int = 9):
    ...
    def update_animation(self, delta_time: float = 1 / 60):
        ...
        #if self.is_flying:
            #self.texture_count()
            #self.texture = self.flying_textures[self.type][self.current_texture][self.direction]
            #return
        ...
        if self.is_attacking:
            ...
            # This if statement turns the direction of animation to 
            # left if the character's change
            # in direction is less than 0 (ie it is moving to the left) 
            # and the character was facing right to start with
            # Meanwhile the elif statement turns the  direction 
            # of animation to the right if 
            # the character's change in direction is positive
            # ( it moves to the right) and also 
            ...
            # It changes the climbing variable depending
            #  if the character is on a ladder or not
            # It also animates the sprite when its 
            ...
            if self.change_y > 0 and not self.on_ladder and not\
                ...
                if not self.is_throwing:
                    ...
                    self.throwing_animation(
                        self.jump_throw_textures[self.type], upper_limit=4)
                return
                ...
                if not self.is_throwing:
                    ...
                    self.throwing_animation(
                        self.jump_throw_textures[self.type], 5)
                return

            
            # This sets the animation to the idle animation
            ...
            # This sets the animation to the walking animation, 
            # looping through the 
...
class Enemy(arcade.Sprite):
    ...
    def __init__(self,folder_name, file_name, scale : int,
        no_of_frames: int):
        ...
        img_path = f"{MAIN_FILE_PATH}Assets/{folder_name}/{file_name}"
        # Loading texture pairs for when the character is idle, 
        # jumping, or falling
        ...
        self.texture = self.walking_textures[0][0]
...
class Minotaur(Enemy):
    ...
    def __init__(self):
        ...
        # and takes on all the variables of it
        super().__init__("Minotaur Enemy", "Minotaur_03_Walking_00", 0.1, 18)
...
class Golem(Enemy):
    ...
    def __init__(self):
        ...
        # Inherits the parent class of Enemy
        # and takes on all the variables of it
        super().__init__("Golem Enemy", "0_Golem_Running_00",0.2, 12)
...
class HealthBar(arcade.Sprite):
    ...
    def draw_health(self):
        ...
        arcade.draw_rectangle_filled(
            self.sprite.center_x, self.sprite.top + 10, self.health_width,
            4 ,color= arcade.color.RED)
        health_width = self.health_width * self.sprite.health / MAX_HEALTH
        arcade.draw_rectangle_filled(
            self.sprite.center_x - 0.5 * self.health_width + 0.5 * 
            health_width, self.sprite.top + 10, health_width, 4,
...
class SpeedPowerup(AnimatedObject):
    ...
    def __init__(self):
        ...
        super().__init__(f"{MAIN_FILE_PATH}Assets/Collectables/speed",
            POWERUP_SIZE, 12,POWERUP_RATE) #Make heart heal size powerup size
...
class Game(arcade.View):
    ...
    def __init__(self, level:int, music:bool, character_num = 0):
        ...
        # Sets the end of map to 0 (Will be changed later,
        # this is only intializing)
        self.map_end = 0
        ...
        # f"{MAIN_FILE_PATH}Assets/Adorable_Little_Chiptune_Loop.mp3", streaming=True)
        self.game_bg_track = arcade.load_sound(
    ...
    def setup(self):
        ...
        # This sets the enemy layer to a list of the 
        # placeholders in the enemy layer
        # and the powerup layer to a list of the 
        # placeholders in its respective layers
        self.enemies_layer = self.tile_map.object_lists[ENEMY_LAYER]
        self.powerups_layer = self.tile_map.object_lists[POWERUP_LAYER]
        #self.checkpoints_layer = self.tile_map.object_lists[CHECKPOINT_LAYER]
        #self.crates_layer = self.tile_map.object_lists[CRATE_LAYER]
        #self.layers_to_setup = [POWERUP_LAYER,COLLECTABLE_OBJECTS_LAYER]

        # This creates the scene with the tilemap, and this will add 
        # all layers of the tilemap as sprtitelists
        # The 2nd line will add the player sprite as a layer
        ...
        # required for it to move. It then adds each enemy to the enemy layer
        # in the map
        ...
        # This sets up the player sprite and the co-ordinates where it 
        # will initially start from
    ...
    def on_draw(self):
        ...
        # This uses the background camera 
        # (whatever comes next will be drawn onto this camera)
        self.bg_cam.use()
        ...
        # This uses the camera (which will follow the sprite) 
        # anything that comes next will be drawn on this camera
        self.cam.use()
        ...
        # This draws the score on a specific co-ordinate on the screen
        # at which it will stay for the rest of the game
        ...
        # This draws the number of knives on a specific
        # co-ordinate on the screen at which
    ...
    def process_keychange(self):
        ...
        # Changes the speed of the character depending on whether 
        # they have the speed boost or not
    ...
    def on_key_press(self, key, modifiers):
        ...
        # This sets the keys pressed to true so when the keychange is 
        # processed the sprite will move. It also keeps track of whether
        # the character is throwing a knife or not
        if key == arcade.key.UP or key == arcade.key.W:
            ...
            # And also check if their change y (and maybe change x too) 
            # is 0 if it is you can change
    ...
    def on_key_release(self, key, modifiers):
        ...
        # This makes sure that the sprite stops moving in a 
        # specific direction every time a key is released
        # It also keeps track of whether the character is 
        # throwing a knife or not
        if key == arcade.key.UP or key == arcade.key.W:
            self.up_pressed = False
            self.jump_needs_reset = False
        elif key == arcade.key.DOWN or key == arcade.key.S:
    ...
    def on_update(self, delta_time):
        ...
        # This moves the player sprite and updates the positioning 
        # of the platforms
        ...
        # if they are in the air and adjusts the on_ladder variables 
        # as they should be
        ...
        # If the speed boost is equipped then this will create a 
        # sprite that will act as a sort of visual thing as to 
        # when the character moves faster
        if self.speed_boost_equipped is True:
            ...
            # For every sprite in the dash layer it will 
            # remove them if they have existed for 3 frames
            # so it looks animated
            for sprite in self.map[DASH_LAYER]:
                if self.boost_frames_since_reset == sprite.start_frame + 3:                 # Add constant
        ...
        if self.can_throw:
            ...
            # then a new knife sprite will be created and added to the layer
            # "Knives". The original direction variable is to track which way 
            # the knife was first going in
            if self.character.is_throwing:
                ...
                if self.character.knives_num > 0:
                    ...
                    self.character.center_x - KNIFE_RANGE,
                    self.character.center_x + KNIFE_RANGE)
                ...
                # If the character cannot throw knives
                else:
                    self.character.is_throwing = False

        
        # IF the player is unable to throw knives because they 
        # threw a knife less than x frames ago, then it will 
        # increase the number of frames between shots
        # If the number of frames is enough then the
        # character will once again be able to throw knives
        else:
        ...
        # If the knife layer exists (there is a knife in it) 
        # then this will run. It will remove an knives that 
        # have collided with the platform or enemy layer
        # and also update the knife speed and rotation
        # (basically animation)
        ...
        for powerup in self.map.get_sprite_list(POWERUP_LAYER):
            ...
            if arcade.check_for_collision(self.character,powerup):
                if powerup.type == "health":
                    if self.hearts.lives < MAX_HEARTS:
                        self.hearts.lives +=1
        ...
        # Updates the enemies animation and also removes them from
        # the enemy list if enemies died
        for enemy in self.map.get_sprite_list(ENEMY_LAYER):
        ...
        # If they do have health they will go to the last checkpoint 
        # they passed but their score will remain intact
        # This also updates the hearts animation after a 
        # character loses a heart
        ...
        for teleport in self.map.get_sprite_list(TELEPORT_LAYER):
            if arcade.check_for_collision(self.character,teleport):
                if teleport == self.map[TELEPORT_LAYER][0]:
                    self.character.center_x = self.map[TELEPORT_LAYER]\
                        [1].center_x + TELEPORT_OFFSET_X
                    self.character.center_y = self.map[TELEPORT_LAYER]\
                        [1].center_y
                else:
                    self.character.center_x = self.map[TELEPORT_LAYER]\
                        [0].center_x - TELEPORT_OFFSET_X
                    self.character.center_y = self.map[TELEPORT_LAYER]\
                        [0].center_y
        ...
        if arcade.check_for_collision_with_list(
            ...
            # Moves onto next level if its not 
            # the highest level already
            if self.level != NUM_OF_LEVELS:
                self.level += 1

            # Runs the game over screen if the user 
            ...
            pass_view = LevelPassed(
                self, self.player_score, current_total_time)
