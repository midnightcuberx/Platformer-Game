...
class PauseView(BasicView):
    ...
    def on_mouse_press(self, _x, _y, _button, _modifiers):
        ...
        main_game = Game(self.game_view.level,self.game_view.music_playing,self.game_view.character.num)
        main_game.setup()
...
class Character(arcade.Sprite):
    ...
    def __init__(self,moving_platforms, character_num = 0):
        ...
        # This is because the chzracter starts on the left side of the map 
        # and therefore should be running right
        self.texture = self.idle_textures[self.num][0]
    ...
    def update_animation(self, delta_time: float = 1 / 60):
        ...
        if self.is_attacking:
            self.texture_count()
            self.texture = self.attacking_textures[self.num][self.current_texture][self.character_direction]
            self.hit_box = self.attacking_textures[self.num][0][self.character_direction].hit_box_points
            ...
            if self.climbing and abs(self.change_y) > 1:
                ...
                if self.current_texture > 9:
                    self.current_texture = 0
            if self.climbing:
                self.texture = self.climbing_textures[self.num][self.current_texture]
                return
            ...
            if self.change_y > 0 and not self.on_ladder and not self.on_moving_platform(self.moving_platforms):
                if not self.is_throwing:
                    self.texture = self.jumping_textures[self.num][self.character_direction]
                else:
                    self.throwing_animation(self.jump_throw_textures[self.num], upper_limit=4)
                return

            elif self.change_y < 0 and not self.on_ladder and not self.on_moving_platform(self.moving_platforms):
                if not self.is_throwing:
                    self.texture = self.falling_textures[self.num][self.character_direction]
                else:
                    self.throwing_animation(self.jump_throw_textures[self.num], 5)
                return
            ...
            if self.change_x == 0:
                if not self.is_throwing:
                    self.texture = self.idle_textures[self.num][self.character_direction]
                else:
                    self.throwing_animation(self.throwing_textures[self.num])
                return

            # This sets the animation to the walking animation, looping through the 
            # number of different walking textures there are
            if not self.is_throwing:
                self.throwing_animation(self.walking_textures[self.num])
            else:
                self.throwing_animation(self.throwing_textures[self.num])
...
class Game(arcade.View):
    ...
    def setup(self):
        ...
        self.character.num = self.character_num
    ...
    def on_key_press(self, key, modifiers):
        ...
        elif key == arcade.key.KEY_1 and self.character.num !=0:
            # And also check if their change y (and maybe change x too) is 0 if it is you can change
            self.character.num = 0
        elif key == arcade.key.KEY_2 and self.character.num!= 1:
            self.character.num = 1
